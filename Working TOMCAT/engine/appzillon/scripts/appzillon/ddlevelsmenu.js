...
arrowpointers:{
	downarrow: [" ",  , ], //[path_to_down_arrow, arrowwidth, arrowheight]
	rightarrow: [" ",  , ], //[path_to_right_arrow, arrowwidth, arrowheight]
	backarrow: [" ",  , ], //[path_to_back_arrow, arrowwidth, arrowheight]
	showarrow: {toplevel: true, sublevel: true} //Show arrow images on top level items and sub level items, respectively?
},  
hideinterval: 200, //delay in milliseconds before entire menu disappears onmouseout.
effects: {enableswipe: true, enableslide: true, enablefade: true, duration: 200},
httpsdivsrc: "blank.htm", //If menu is run on a secure (https) page, the IFRAME shim feature used by the script should point to an *blank* page *within* the secure area to prevent an IE security prompt. Specify full URL to that page on your server (leave as is if not applicable).
...
addpointer:function(target, iclass, imginfo, BeforeorAfter){
	var pointer=document.createElement("i")
	pointer.src=imginfo[0]
	pointer.style.width=imginfo[1]+"px"
	pointer.style.height=imginfo[2]+"px"
	if(iclass=="rightarrowpointer"){
		pointer.style.left=target.offsetWidth-imginfo[2]-2+"px"
	}
	pointer.className=iclass
...
addshimmy:function(target){
	...
	shim.className="dddivshim"
	shim.setAttribute("src", location.protocol=="https:"? this.httpsdivsrc : "about:blank")
...
buildmenu:function(mainmenuid, header, submenu, submenupos, istoplevel, dir){
	...
	this.addEvent(header, function(e){ //mouseover event
		if (ddlevelsmenu.ismobile || !ddlevelsmenu.isContained(this, e)){
			...
			// Bug 50959 - The submenu is being shown out of the window if the page is scrolled down to the bottom
			else if(submenubottomedge > window.innerHeight){
                menutop = this._offsets.top-(submenubottomedge-ddlevelsmenu.docheight-this._dimensions.h)-this._dimensions.h + (window.innerHeight-ddlevelsmenu.docheight);
            }
...
buildmobilemenu:function(menuid, ul, dir){
	...
	var flattened = flattenul(ul, false,
		function(i, header, subul){ // loop through header LIs and sub ULs
			...
			breadcrumb.innerHTML = '<i ' + ddlevelsmenu.arrowpointers.backarrow[0] + ' class="backarrowpointer" /> ' + header.getElementsByTagName('p')[0].firstChild.nodeValue
	...
	for (var i=0; i<alllinks.length; i++){
		if (alllinks[i].getAttribute('rel')){
			...
			if (this.arrowpointers.showarrow.toplevel)
				this.addpointer(menuitem, arrowclass, arrowpointer, "after")
			...
			for (var c=0; c<subuls.length; c++){
				...
				if (this.arrowpointers.showarrow.sublevel)
					this.addpointer(parentli.getElementsByTagName("a")[0], "rightarrowpointer", this.arrowpointers.rightarrow, "after")
...
setup:function(mainmenuid, dir){
	...
	ddlevelsmenu.init(mainmenuid, dir, resp);
